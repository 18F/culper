...
describe('The ForeignBornDocuments component', () => {
  it('renders focus and blur', () => {
    ...
    const onBlur = () => { blurs++ }
    const onFocus = () => { focus++ }
    const component = mount(<ForeignBornDocuments onBlur={onBlur} onFocus={onFocus} />)

    component.find('.born input').first().simulate('blur')
    component.find('.born input').first().simulate('focus')
    expect(blurs).toBe(1)
    expect(focus).toBe(1)
  })
