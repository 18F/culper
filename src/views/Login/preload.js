...
export const persist = (dispatch, application = {}) => {
  navigationWalker((path, child) => {
    if (path.length && path[0].store && child.store && child.validator) {
      ...
      if (data) {
        api.save(schema(`${sectionName.toLowerCase()}/${subsectionName.toLowerCase()}`.replace(/\//g, '.'), data, false))
      }
    }
  })
}
...
export const preload = (dispatch, application = {}) => {
  ...
  if (application.Substance && application.Substance.ReceivedCounselings) {
    dispatch(updateApplication('Substance', 'ReceivedCounselings', application.Substance.ReceivedCounselings))
  }
  if (application.Substance && application.Substance.NegativeImpacts) {
    dispatch(updateApplication('Substance', 'NegativeImpacts', application.Substance.NegativeImpacts))
  }
  if (application.Substance && application.Substance.OrderedCounselings) {
    dispatch(updateApplication('Substance', 'OrderedCounselings', application.Substance.OrderedCounselings))
  }
  if (application.Substance && application.Substance.VoluntaryCounselings) {
    dispatch(updateApplication('Substance', 'VoluntaryCounselings', application.Substance.VoluntaryCounselings))
  }
  if (application.Substance && application.Substance.DrugClearanceUses) {
    dispatch(updateApplication('Substance', 'DrugClearanceUses', application.Substance.DrugClearanceUses))
  }
  if (application.Substance && application.Substance.PrescriptionUses) {
    dispatch(updateApplication('Substance', 'PrescriptionUses', application.Substance.PrescriptionUses))
  }
  if (application.Substance && application.Substance.OrderedTreatments) {
    dispatch(updateApplication('Substance', 'OrderedTreatments', application.Substance.OrderedTreatments))
  }
  if (application.Substance && application.Substance.DrugPublicSafetyUses) {
    dispatch(updateApplication('Substance', 'DrugPublicSafetyUses', application.Substance.DrugPublicSafetyUses))
  }
  if (application.Substance && application.Substance.DrugInvolvements) {
    dispatch(updateApplication('Substance', 'DrugInvolvements', application.Substance.DrugInvolvements))
  }
  if (application.Substance && application.Substance.DrugUses) {
    dispatch(updateApplication('Substance', 'DrugUses', application.Substance.DrugUses))
  }
  if (application.Substance && application.Substance.VoluntaryTreatments) {
    dispatch(updateApplication('Substance', 'VoluntaryTreatments', application.Substance.VoluntaryTreatments))
  }
