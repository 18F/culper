...
context('Information About You Section', () => {
  beforeEach(() => {
    ...
    cy.fixture('users').then((users) => {
      ...
      .get('.section-header').contains('Section 1: Information about you')
  ...
  describe('Fill out section successfully', () => {
    it('fills in basic applicant data', () => {
      cy.isLocation('/form/identification/name')
        ...
        .get('.section-header').contains('Your contact information')
        .fixture('applicantData').then((applicant) => {
        ...
        .get('.section-header').contains('Your identifying information')
        .fixture('applicantData').then((applicant) => {
  ...
  describe('Verify error messages', () => {
    it('shows all errors in review', () => {
      cy.isLocation('/form/identification/name')
        .get('a.section-link[href="/form/identification/review"]').contains('Review').click()
        .get('.view-review .error-list').within(($errorList) => {
          cy.get('.usa-alert-body .usa-alert-heading').contains('Here is a list of the 12 questions with issues')
            .get('h4').contains('Full name')
            .get('li a').contains('Provide your full name')
            .get('h4').contains('Date of birth')
            .get('li a').contains('Provide your date of birth')
            .get('h4').contains('Place of birth')
            .get('li a').contains('Provide your place of birth')
            .get('li a').contains('Were you born in the United States?')
            .get('h4').contains('Social security number')
            .get('li a').contains('Provide your U.S. Social Security Number')
            .get('h4').contains('Other names used')
            .get('li a').contains('Have you used any other names?')
            .get('h4').contains('Phone number 1')
            .get('li a').contains('United States telephone number')
            .get('h4').contains('Your identifying information')
            .get('li a').contains('Height')
            .get('li a').contains('Weight')
            .get('li a').contains('Hair color')
            .get('li a').contains('Eye color')
            .get('li a').contains('Select your sex')
        })
        .isSectionLinkMarkedInvalid('/form/identification/name')
        .get('.applicant-name').within(($applicantName) => {
          cy.get('.section-header').contains('Full name')
            .get('.title').contains('Provide your full name')
            .get('.usa-alert-error .usa-alert-body div[data-i18n="error.name.first.required"]').within(($alertBody) => {
            cy.get('h5').contains('Oops, there’s a problem.')
              .get('p').contains('If your first name is a single letter, please select the "Initial only" checkbox and type the letter. 100 character limit.')
          })
            .get('.name').within(($name) => {
            cy.get('.first').should('have.class', 'usa-input-error')
              .get('.middle').should('have.class', 'usa-input-error')
              .get('.last').should('have.class', 'usa-input-error')
          })
        })
        //TODO .isSectionLinkMarkedInvalid('/form/identification/othernames')
        .get('.other-names').within(($otherNames) => {
          cy.get('.usa-alert-error .usa-alert-body div[data-i18n="error.required"]').contains('Your response is required')
            .get('.blocks').should('have.class', 'usa-input-error')
        })
        .isSectionLinkMarkedInvalid('/form/identification/contacts')
        .get('.contact .telephone-collection').within(($telephoneCollection) => {
          cy.get('.usa-alert-error .usa-alert-body div[data-i18n="error.telephone.required"]').contains('Your response is required')
            .get('.telephone-number-fields div.number.three').each(($el) => {
              cy.wrap($el).should('have.class', 'usa-input-error')
            }).then(($list) => {
              expect($list).to.have.length(2)
            })
            .get('.telephone-number-fields div.number.four').should('have.class', 'usa-input-error')
            .get('.blocks').should('have.class', 'usa-input-error')
        })
        .isSectionLinkMarkedInvalid('/form/identification/birthdate')
        .get('.birthdate').within(($birthdate) => {
          cy.get('.usa-alert-error .usa-alert-body div[data-i18n="error.date.required"]').within(($alertBody) => {
            cy.get('h5').contains('Your response is required')
              .get('span').contains('All parts of the date are required even if it is estimated.')
          })
          .get('.datecontrol').within(($datecontrol) => {
            cy.get('.month div').should('have.class', 'usa-input-error')
              .get('.day div').should('have.class', 'usa-input-error')
              .get('.year div').should('have.class', 'usa-input-error')
          })
        })
        .isSectionLinkMarkedInvalid('/form/identification/birthplace')
        .get('.applicant-birthplace').within(($birthplace) => {
          cy.get('.usa-alert-error .usa-alert-body div[data-i18n="error.location.toggleablelocation.required"]').contains('Your response is required')
            //TODO .get('.usa-alert-error .usa-alert-body div[data-i18n="error.toggleablelocation.required"]').contains('Your response is required')
            .get('.blocks').should('have.class', 'usa-input-error')
        })
        .isSectionLinkMarkedInvalid('/form/identification/ssn')
        .get('.applicant-ssn').within(($ssn) => {
          cy.get('.usa-alert-error .usa-alert-body div[data-i18n="error.ssn.required"]').contains('Your response is required')
            .get('.first').should('have.class', 'usa-input-error')
            .get('.middle').should('have.class', 'usa-input-error')
            .get('.last').should('have.class', 'usa-input-error')
        })
        .isSectionLinkMarkedInvalid('/form/identification/physical')
        .get('.physical').within(($phsyical) => {
          cy.get('.field[aria-label="Height"] .usa-alert-error .usa-alert-body div[data-i18n="error.height.required"]').contains('Your response is required')
            .get('.height .feet div').should('have.class', 'usa-input-error')
            .get('.height .inches div').should('have.class', 'usa-input-error')
            .get('.field[aria-label="Weight"] .usa-alert-error .usa-alert-body div[data-i18n="error.weight.required"]').contains('Your response is required')
            .get('.weight .pounds div').should('have.class', 'usa-input-error')
            .get('.field[aria-label="Hair color"] .usa-alert-error .usa-alert-body div[data-i18n="error.required"]').contains('Your response is required')
            .get('.field[aria-label="Hair color"] .blocks').should('have.class', 'usa-input-error')
            .get('.field[aria-label="Eye color"] .usa-alert-error .usa-alert-body div[data-i18n="error.required"]').contains('Your response is required')
            .get('.field[aria-label="Eye color"] .blocks').should('have.class', 'usa-input-error')
            .get('.field[aria-label="Select your sex"] .usa-alert-error .usa-alert-body div[data-i18n="error.required"]').contains('Your response is required')
            .get('.sex .blocks').should('have.class', 'usa-input-error')
        })
    })
  })
})
