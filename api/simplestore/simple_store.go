...
// CreateaApplication saves an application in the db
func (s SimpleStore) CreateApplication(app api.Application) error {
	...
	_, saveErr := s.db.Exec(saveQuery, app.AccountID, string(json))
	if saveErr != nil {
...
func (s SimpleStore) UpdateApplication(app api.Application) error {
	...
	result, saveErr := s.db.Exec(updateQuery, string(json), app.AccountID)
	if saveErr != nil {
...
func (s SimpleStore) SaveSection(section api.Entity, accountID int) error {
	...
	app, loadErr := s.LoadApplication(accountID)
	if loadErr != nil {
	...
	return s.UpdateApplication(app)
}
...
func (s SimpleStore) LoadApplication(accountID int) (api.Application, error) {
	...
	selectErr := s.db.Get(&row, selectQuery, accountID)
	if selectErr != nil {
