...
func statusRun(cmd *Command, args ...string) {
	...
	min := int64(0)
	max := int64((1 << 63) - 1)
	migrations, e := migration.CollectMigrations(conf.MigrationsDir, min, max)
	if e != nil {
		log.Fatal(e)
	}

	db, e := migration.OpenDBFromDBConf(conf)
	if e != nil {
	...
	if _, e := migration.EnsureDBVersion(conf, db); e != nil {
		log.Fatal(e)
	}

	fmt.Printf("plucked: status for environment '%v'\n", conf.Env)
