...
func createRun(cmd *Command, args ...string) {
	if len(args) < 1 {
		log.Fatal("plucked create: migration name required")
	}
	...
	n, err := migration.CreateMigration(args[0], migrationType, conf.MigrationsDir, time.Now())
	if err != nil {
		log.Fatal(err)
	}
